...
m1_empl <- glmer(transition_successful ~ 1 + 
...
m3_empl <- glmer(transition_successful ~ 1 + 
                   age + I(age^2) + # assuming nonlinearity of the age effect
                   sex + edu_lvl + area + in_education +  ses5 + # controls
                   O + C + E + A + ES + # NCS
                   (1|region)  + (1|idind) + (1|age_factor), # random effects
...
m4_empl <- glmer(transition_successful ~ 1 + 
                   age + I(age^2) + # assuming nonlinearity of the age effect
...
summary(m4_empl_logit)
margins(m4_empl_logit)

me_c <- predict_response(m4_empl , terms = c("C", "ses5"), type = "random")
plot(me_c, show_ci = FALSE)

me_e <- predict_response(m4_empl , terms = c("E", "ses5"), type = "random")
plot(me_e, show_ci = FALSE)

me_es <- predict_response(m4_empl , terms = c("ES", "ses5"), type = "random")
plot(me_es, show_ci = FALSE)
...
m4_empl_coefs =
  ...
  mutate(Estimate = as.numeric(exp(Estimate))) %>%
...
plot_ses_logit =
  ggplot(m4_empl_logit_coefs, aes(Estimate, ses5)) +
  geom_point(size = 7, color = "lightblue") +
  geom_text(aes(label = round(Estimate, 2)), size = 2, color = "black") +
  geom_vline(xintercept = 1, linetype = "solid",  color = "black") +
  theme_bw() +
